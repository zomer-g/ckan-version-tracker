...
function JoinHelper({ profile, columns, onUseSql }: {
  ...
  const btn: React.CSSProperties = {
    ...
    border: "1px solid var(--primary)", background: "var(--bg)", color: "var(--primary)",
  };
  return (
    <div style={{ marginTop: "0.6rem", padding: "0.5rem 0.7rem", border: "1px dashed var(--primary)", borderRadius: 6 }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={chip("var(--tint-sky-bg)", "var(--tint-sky-fg)")}>{colNames.length} שדות</span>
        ...
        <div style={{ padding: "0 0.7rem 0.7rem" }}>
          ...
          {(tags.length > 0 || keywords.length > 0) && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: "0.6rem" }}>
              {tags.map((t) => (
                <span key={`t-${t}`} style={chip("var(--tint-violet-bg)", "var(--tint-violet-fg)")}>{t}</span>
              ))}
              {keywords.map((k) => (
                <span key={`k-${k.token}`} style={chip("var(--tint-neutral-bg)", "var(--tint-neutral-fg)")} title={`${k.count} מופעים`}>
                  {k.token}
                </span>
              ))}
            </div>
          ...
          <div tabIndex={0} role="region" aria-label="פרופיל הטבלה" className="scroll-region" style={{ overflowX: "auto", border: "1px solid var(--border)", borderRadius: 4, background: "var(--bg)" }}>
              ...
              <thead>
                <tr style={{ background: "var(--surface-2)" }}>
                  {["שדה", "סוג", "טווח / min–max", "ישות", "מילוי", "ערכים ייחודיים", "תיאור"].map((h) => (
                    <th scope="col" key={h} style={{ textAlign: "start", padding: "0.3rem 0.5rem", borderBottom: "2px solid var(--border)" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={name} style={{ borderBottom: "1px solid var(--border)" }}>
                      ...
                      <td style={{ padding: "0.3rem 0.5rem" }}>
                        ...
                        {c.native && <span style={{ ...chip("var(--tint-neutral-bg)", "var(--tint-neutral-fg)"), marginInlineStart: 4 }}>native</span>}
                      </td>
                      <td style={{ padding: "0.3rem 0.5rem" }} dir="ltr">
                        {rangeText(c) || "—"}
                        {df?.python && (
                          <span style={{ ...chip("var(--tint-neutral-bg)", "var(--tint-neutral-fg)"), marginInlineStart: 4 }} title={df.ambiguous ? "פורמט תאריך — ייתכן dd/mm מול mm/dd" : "פורמט תאריך שזוהה"}>
                      ...
                      <td style={{ padding: "0.3rem 0.5rem", whiteSpace: "normal", maxWidth: 260, color: "var(--text-muted)" }}>
                        {descOf(name) || ""}
                      </td>
                    </tr>
                  );
