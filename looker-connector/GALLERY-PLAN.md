...
## סטטוס (עדכון 25.7.2026)

| שלב | מצב |
|---|---|
| א' — עמודים ציבוריים | ✅ חיים ב-www.z-g.co.il (‏over-looker / -privacy / -terms / -support) |
| ב' — מניפסט | ✅ כל שדות החובה, מצביע על הכתובות הקנוניות |
| ג' — PR לרישום | ✅ הוגש: googledatastudio/ds-data-registry#1282 — ממתין למיזוג |
| ד' — GCP + אימות OAuth | ✅ פרויקט over-looker (מס' 1042210751499), In production, **Branding verified** — מסך ההסכמה נקי וממותג |
| ה' — הכנת הסקריפט | ✅ הושלם: deployment ‏"Production" ‏(@2, גם הקישור באתר עליו) + שיתוף Viewer לשני צוותי גוגל + מניפסט גלוי בעורך (25.7.2026) |
| ו' — טופס הגשה | ⏳ **הצעד היחיד שנותר** — אחרי מיזוג ה-PR (#1282) |

לקח מהמעבר: החלפת פרויקט ה-GCP איפסה את ההרשאות — עד לחיצת **Publish app**
(מצב Testing) כל אישור חדש נכשל ב-403; לזכור בכל פרויקט עתידי.

---

מצב פתיחה (24.7.2026): המחבר **חי ופועל** כ-by-link — v1 מפורסם בעמוד ה-API של
