...
### וגם צד GOVSCRAPER (`ea58d57`)
`ocoi_pipeline.py` מרוקן עכשיו את התור: `reconvert_once()` רץ **לפני** מנת
הייבוא בכל סבב (התור לא מווסת, ובקשה שאדם עשה ביד לא צריכה לחכות מאחורי 25
מסמכים חדשים) ואינו דורש מפתח DeepSeek — ההמרה היא מה שביקשו, ו-OVER משאיר את
החילוץ ב-pending כשלא נשלח. מסמך שעדיין לא ניתן לקריאה **נשאר מסומן** ולא נדחף
ריק: סימנו אותו כי הטקסט גרוע, ודחיפה ריקה היתה מנקה את הסימון ומחליפה את
הטקסט בכלום — ובניגוד לייבוא, שום discovery לא מציע שוב מסמך שכבר מאוחסן.
כשל בשלב הזה עטוף כך שלא יפיל את מנת הייבוא.

> ⏳ **נשאר:** `GET /api/worker/ocoi-config` (הפרומפט לחילוץ) עדיין לא נצרך —
> עריכת הפרומפט באדמין נשמרת ואינה משפיעה על החילוץ בפועל. שינוי קטן, אבל
> הפרומפט הוא הכפתור עם רדיוס הפגיעה הגדול ביותר בצינור ומגיע לו commit משלו.
>
> ⏳ **ופעולה שלך:** להפעיל מחדש את `worker_supervisor.py` — העדכון העצמי מושך
> קוד אך אינו מריץ מחדש את הסופרוויזר, ודמון ה-OCOI מופעל רק באתחולו. הצינור
> אינו רץ מאז 19.8.

---

## ✅ Phase 6 שלב ד׳ — הכל מוכן לכיבוי (1.9.2026)

### מה נבנה

**קישורי העומק של ocoi.org.il שורדים את הכיבוי.** זו הנקודה הקריטית: מודל
האמינות של OCOI הוא שכל טענה מקושרת למסמך שממנו חולצה, ושרת ה-MCP שלו **מורה
למודלים לצטט** `https://www.ocoi.org.il/document?id=…` ו-`/entity?type=&id=`.
הכתובות האלה כבר יושבות בטקסטים שאי אפשר לערוך בדיעבד. להפנות את הדומיין
ל-OVER בלי המסלולים האלה = להפוך כל ציטוט שהפרויקט אי פעם הפיק ל-404, ברגע
המעבר.

| מסלול ישן | יעד |
|---|---|
| `/entity?type=&id=` | `/projects/ocoi?tab=graph&type=&id=` |
| `/document?id=` | `/projects/ocoi?tab=documents&doc=` |
| `/search?q=` (רק ל-host הישן) | `/projects/ocoi?q=` |

`/search` מוגבל ל-host הנוטש בלבד — «entity» ו-«document» הם שמות ששום דבר אחר
כאן לא רוצה, אבל `/search` הוא שם ש-OVER עשוי לרצות לעצמו. בדיקת ה-host היא
**קבוצה מדויקת ולא endswith**: `myocoi.org.il` מסתיים ב-`ocoi.org.il`, והבדיקה
תפסה את זה.

לקישור למסמך לא היה לאן לנחות, אז לשונית המסמכים מקבלת `?doc=<id>` ומציגה מסמך
יחיד עם דרך חזרה לרשימה. ציטוט שנוחת על רשימה לא מסוננת לא באמת נפתר.

**תוסף הכרום** מצביע ל-`over.org.il/api/ocoi`. שני ה-endpoints פורטו עם אותה
חתימה ואותה מעטפת `{status, data, meta}`, ולכן רק הבסיס משתנה. מה שלא מגיע
בחינם: ערך ב-`chrome.storage` **גובר** על ה-DEFAULTS, אז שחרור ברירת מחדל חדשה
לבדה היה משאיר כל התקנה קיימת מצביעה לשרת מת — בשקט, בלי שגיאה שהמשתמש יכול
לפעול לפיה. ה-worker מתקן ערך ישן ב-install וב-startup ומנקה את מטמון השכנים.
גרסה 0.2.0, `dist/ocoi-extension-v0.2.0.zip` מוכן להעלאה, וטקסטי החנות עודכנו
(ביקורת דוחה נימוק שמתאר הרשאות שהמניפסט כבר לא מבקש).

**קצב:** הוורקר סוקר כל 4 שעות ושער המועמדים של OVER ירד מ-5ל-3 שעות. שני
המספרים הם זוג — סקירה איטית מהשער והקורפוס גדל פעם אחת לסקירה; מהירה ממנו ורוב
המעברים נדחים לחינם.

### הרצה מאומתת מקצה לקצה
`--reconvert` → 0 (התור ריק, אבל הפַּס הזדהה מול OVER). מנת ייבוא של 3 מסמכים →
`imported=3, errors=0`, OCR פעיל, DeepSeek עונה, R2 שמר, והסטטוס נכתב `extracted`.
הקורפוס 2,974 → 2,977 — הצמיחה הראשונה מאז 19.8.

### רשימת הכיבוי — לפי הסדר

1. **גיבוי 6 טבלאות ה-auth/billing.** `python -m scripts.ocoi_export_auth_tables`
   אחרי `$env:OCOI_RENDER_DATABASE_URL = "<External Connection String>"`.
   הן מעולם לא הוגרו (טוקנים ומזהי Stripe, וה-append DB מגבה קונסולה ציבורית),
   ולכן קיימות **רק** במופע ה-Render החינמי שעומד להימחק. `pg_dump` לא מותקן
   כאן, ולכן הסקריפט קורא שורות ישירות. להשוות ספירות מול הדשבורד לפני מחיקה.
2. **תוסף הכרום לחנות** — הפריט עם זמן ההמתנה הארוך ביותר (ביקורת של ימים עד
   שבועות). **חייב להתפרסם לפני הכיבוי**, לא אחריו.
3. **מחבר ה-MCP** → `over.org.il/ocoi/mcp` (המחבר הנוכחי עדיין מצביע ל-ocoi.org.il
   — נמדד: הוא מחזיר את הספירות הישנות ו-`source_site: ocoi.org.il`).
4. **DNS ב-internic** — `ocoi.org.il` → `over.org.il`. אז המסלולים שלמעלה
   מתחילים לעבוד לדומיין הישן, ואפשר לאמת את `/search` מקצה לקצה (מכאן אי אפשר:
   Cloudflare דוחה כותרת Host מזויפת מול ה-SNI).
5. **כיבוי ב-Render** — שירות `ocoi` ואז מסד `ocoi-db`.

> ⏳ נשאר פתוח במכוון: `GET /api/worker/ocoi-config` (פרומפט החילוץ) עדיין לא
> נצרך בוורקר, כלומר עריכת הפרומפט באדמין נשמרת ואינה משפיעה על החילוץ. שינוי
> קטן, אבל הפרומפט הוא הכפתור עם רדיוס הפגיעה הגדול ביותר בצינור.
>
> ⏳ ושאלה שכדאי להכריע: מקור התוסף יושב בריפו `ocoi`, שעומד להיארכב. אם התוסף
> ימשיך להתפרסם, כדאי להוציא אותו לריפו משלו (כמו ykpubdata-scraper).

---

## ✅ Phase 6 — מוכן לכיבוי (1.9.2026, סבב אחרון)

### ארבע הכרעות שהתקבלו, ומה הן שינו

| הכרעה | תוצאה |
|---|---|
| **DNS לא עובר** — ocoi.org.il יקבל אתר סטטי | ההפניות ב-OVER **לא** יקבלו את התעבורה הישנה. האתר הסטטי חייב לשאת אותן — נבנה. |
| **התוסף יכול לא לעבוד בינתיים** | הקוד פורט ונארז (0.2.0), אך אינו חוסם את הכיבוי. |
| **הגיבוי — בוצע** | 589 שורות ב-6 טבלאות. |
| **MCP קריטי** | תוקן ואומת בצד השרת; נותרה פעולת ממשק אחת. |

### הגיבוי בוצע — והמסד ריק מחובות
`OCOI_RENDER_SRC_URL` שב-.env הוא אותו DSN שההגירה קראה, ולכן לא נדרש סוד חדש.

```
users 12 · oauth_clients 10 · oauth_authorization_codes 6
oauth_refresh_tokens 492 · billing_accounts 10 · usage_events 59   = 589
```

⚠ `backups/` **לא היה ב-.gitignore** — 492 refresh tokens ישבו כקובץ untracked
שכל `git add -A` היה דוחף לריפו ציבורי. תוקן לפני שנגענו בכל השאר.

**השוואת כל טבלה במסד ה-Render מול OVER:** כל 14 טבלאות הדאטה קיימות ב-OVER
בספירה **שווה או גבוהה** (documents ‎+6, persons ‎+6, relationships ‎+9 —
הוורקר ממשיך למלא), registry_records 797,900 = 797,900. ששת טבלאות ה-auth
מגובות. **אין שורה אחת שנשארת רק ב-Render.**

### ה-MCP המחליף אומת חי
`over.org.il/ocoi/mcp` עם service token: 10 כלים, ו-`stats` מחזיר את הקורפוס
**העדכני** (2,977 מסמכים) לעומת 2,971 שהמחבר הישן מחזיר. השרת מוכן; מה שנותר
הוא הוספת המחבר בהגדרות Claude — פעולת חשבון שאי אפשר לבצע מהקוד.

### האתר הסטטי — `Ocoi/static-site/`
נושא את קישורי הציטוט, כי ה-DNS נשאר במקומו:

| כתובת ישנה | יעד |
|---|---|
| `/document?id=X` | `?tab=documents&doc=X` |
| `/entity?type=T&id=X` | `?tab=graph&type=T&id=X` |
| `/search?q=Q` | `?q=Q` |
| `/persons\|companies\|associations\|domains/X` | אותה ישות בגרף |
| `/api/v1/*` | `/api/ocoi/*` |

נתיבי ה-query מקבלים דף הפניה משלהם כי היעד **משנה צורת פרמטרים**
(`id`→`doc` ותוספת `tab`), וכלל rewrite לא יכול לבטא זאת. נתיבי הנתיב יושבים
ב-`_redirects` (Netlify/Cloudflare) וב-`404.html` (Render/GitHub Pages), כך
ששני סוגי אחסון מתנהגים נכון. **אומת מקצה לקצה מול האתר החי.**

### מה שנותר לך — ורק אז לכבות
1. לפרוס את `static-site/` כ-Static Site ולהעביר אליו את הדומיין.
2. להוסיף את המחבר `over.org.il/ocoi/mcp` בהגדרות Claude.
3. **ואז** למחוק את שירות `ocoi` ואת `ocoi-db`.

> ⏳ פתוח במכוון: `ocoi-config` (פרומפט החילוץ) עדיין לא נצרך בוורקר; התוסף
> ארוז ולא פורסם; ומקור התוסף יושב בריפו שעומד להיארכב.
