...
_STORY_PAGES = {
    ...
    # Both .pptx decks are generated by scripts/build_*_deck.py from a content
    # module, so a figure in them can be corrected and the file rebuilt rather
    # than hand-edited in PowerPoint. The timeline deck presents the same story
    # as over-timeline.html, which stays the source of truth for it.
    "spatial-deck.pptx": ("over-spatial-deck.pptx", _PPTX,
                          "over-spatial-deck.pptx"),
    "timeline-deck.pptx": ("over-timeline-deck.pptx", _PPTX,
                           "over-timeline-deck.pptx"),
}
