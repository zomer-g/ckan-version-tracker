...
async def _build_catalog_uncached(db: AsyncSession) -> list[dict]:
    ...
    out.extend(await _over_index_records())
...
_OVER_TITLES = {
    "over_settlements": "גרסאות לעם — יישובים (מדד הלמ״ס)",
    "over_settlement_aliases": "גרסאות לעם — הטיות שמות יישובים",
    "over_authorities": "גרסאות לעם — רשויות מקומיות",
...
async def _over_index_records() -> list[dict]:
    ...
    These are generated BY גרסאות לעם — the settlement/authority reference index
    (official names + inflections) and the table profiles — not an original
    public source. Badged as the "גרסאות לעם" processed source in /data, a series
    of index tables meant to be JOINed against the real datasets."""
    try:
        cols_by_table = await append_store.schema_table_columns("public")
        est = await append_store.list_public_tables()  # only counts append_*; over_* absent → None
...
async def table_detail(table: str, db: AsyncSession) -> dict | None:
    ...
    if rec["kind"] in ("index", "odata", "ocal", "over"):
