...
SERVER_INSTRUCTIONS = (
    ...
    "(4א) עסקאות נדל\"ן: append_taxes_nadlan_full_f41fb496_fd06f5ae, 3.84 מיליון "
    "עסקאות מיסוי מקרקעין. גוש וחלקה שם הם טקסט של מספר שלם ללא אפסים מובילים "
    "(gush = '7104'), וזה גם האינדקס — חברו אליהם ולא ל-parcel_key, כי המאגר "
    "אינו מפרסם תת-גוש. deal_date הוא DD/MM/YYYY ו-to_date אינה IMMUTABLE, "
    "ולכן למיון ולטווחי תאריכים השתמשו בצורה הממוינת "
    "substr(deal_date,7,4)||substr(deal_date,4,2)||substr(deal_date,1,2) — "
    "היא גם האינדקס. ל-17.5% מהשורות אין settlement_code, ולכן סננו לפי "
    "settlement (השם). השוו בחציון (percentile_disc) ולא בממוצע: שורה אחת "
    "יכולה להיות דירה אחת או בניין שלם.\n"
    "(4ב) אזור סטטיסטי (א\"ס): שכבת 2022 היא "
    "append_cbs_pub_file_42a1e1f0_693e114c ובה geom, ולכן שיוך נקודה לאזור "
    "הוא ST_Contains ולא צירוף לפי קוד יישוב. המדד החברתי-כלכלי מתפרסם רק "
    "על חלוקת 2011 (append_cbs_pub_file_afb48290_5fa5cab4), שהיא גיאומטריה "
    "אחרת: אל תחברו בין השתיים לפי קוד.\n\n"
    ...
    "שאילתה שנועדה למפה של /data — שלושה כללים שבלעדיהם היא רצה ולא מצטיירת:\n"
    "(א) החזירו ST_AsText(geom) ולא geom. המפה מזהה גאומטריה לפי התוכן, וערך "
    "geom גולמי חוזר כ-WKB הקסדצימלי (0106000020E610...) שאינו נראה כמו WKT — "
    "כך שלא נמצאת עמודת גאומטריה כלל וכפתור המפה נשאר כבוי.\n"
    "(ב) הצבע נקבע בבורר 'צבע לפי' שבממשק, לא בשאילתה. עמודת color עם ערכי "
    "hex לא עושה דבר; מה שנדרש הוא עמודת קטגוריה אחת עם מעט ערכים (למשל "
    "layer) שהמשתמש יבחר בה.\n"
    "(ג) התצוגה נחתכת ב-" + str(MAX_SQL_ROWS) + " שורות לפי ה-ORDER BY. "
    "ב-UNION של כמה שכבות זה מוחק בשקט את השכבות האחרונות — צמצמו את ההיקף "
    "(למשל יישוב אחד) עד שהסך קטן מהתקרה, ואמרו זאת למשתמש.\n"
    "(ד) אל תפשטו גאומטריה מיוזמתכם. ST_Simplify* משנה את הצורה: על שכבת "
    "השכונות של ירושלים, סף של 0.00015 (כ-15 מ') הוריד פוליגון מ-95 נקודות "
    "ל-28, הזיז את הקו עד 15 מ' ושינה שטח ב-0.8% — הבדל שלא נראה בזום עירוני "
    "אבל צועק בזום של רחוב, ומי שמשווה גבולות בין שתי חלוקות משווה בדיוק שם. "
    "החזירו את הגאומטריה כפי שהיא. אם התשובה כבדה מדי — צמצמו שורות (יישוב "
    "אחד, פחות שכבות) לפני שאתם נוגעים בצורה, ואם בכל זאת פישטתם כי המשתמש "
    "ביקש — אמרו לו את הסף במטרים ושהגבולות זזו.\n\n"
